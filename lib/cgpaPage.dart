...
class GPAcalcstate extends State<cgpaCalc> {
  ...
  void initState() {
    ...
    _sgpaController = List<int>.filled(widget.n, 0, growable: true);

    _creditController =List<int>.filled(widget.n, 0, growable: true);
...
//changed this part to navigator and builder
                if(insertedValue==true)
                  Navigator.push(context,
                    MaterialPageRoute(
                      builder: (context) =>  ResultPage(cgpa),
                    ),
                  );
                else{
                  showAlertBox();
